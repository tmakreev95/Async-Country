    ...
    .then(data => {
      ...
      if (neighbours.length === 0) {
        ...
        neighbours.forEach(neighbour => {
          const apiUrl = `https://restcountries.com/v2/alpha/${neighbour}`;
          return fetch(apiUrl)
...
const whereAmI = function (lat, lng) {
  fetch(`https://geocode.xyz/${lat},${lng}?geoit=json`)
    .then(response => {
      if (!response.ok) {
        throw new Error(`Request error from API: ${response.status}`);
      }
      return response.json()
    })
    .then(data => {
...
/* Even Loop Practice */
//Regular function call - 1
// console.log('Test start');
//Callback queue - 4
// setTimeout(() => console.log('0 sec timer'), 0);
//Microtask queue - 3 (Microtasks queue has higher prio than Callback queue)
// Promise.resolve('Reseloved promise 1')
...
/* Promisifying the Geolocaiton API */
...
// whereAmIAsync().then(city => console.log(city)).catch(err => console.log(err))
